...
const guildconfig = new Keyv("sqlite://db.sqlite", {
...
module.exports = {
  ...
  async execute(message, client) {
    var authorDB = await userdata.get(
      `author_${message.author.id}_${message.guild.id}`
    );

    if (message.author.bot) return;

    if (message.channel.type === "dm")
      return message.channel.send("Sorry, I dont do dms!");
    ...
    var antispam = await userdata.get(
      `antispam_${message.author.id}_${message.guild.id}`
    );
    ...
    if (!message.member.roles.cache.has("831607805802577960")) {
      ...
      if (found == true) {
        var linkwarns = await userdata.get(
          `antilink_${message.author.id}_${message.guild.id}`
        );
        ...
        const success = new Discord.MessageEmbed()
          ...
          .setDescription(
            `${message.author.username}#${message.author.discriminator} (ID: ${message.author.id}) was warned for posting links`
          )
          .setTimestamp()
