...
module.exports = {
  ...
  async execute(message, client, args, userdata) {
    ...
    if (!offender)
      return message.channel.send("You didnt mention anyone to warn!");

    if (offender.id == message.author.id)
      return message.channel.send("Hey you cant warn yourself!");

    var warns = await userdata.get(
      `warnings_${offender.id}_${message.guild.id}`
    );
    ...
    const success = new Discord.MessageEmbed()
      ...
      .setDescription(
        `${offender.user.username}#${offender.user.discriminator} (ID: ${offender.id}) was warned for: ${reason}`
      )
      .setTimestamp()
      .setFooter(
