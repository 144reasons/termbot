...
module.exports = {
  ...
  async execute(message, client, args, userdata) {
    ...
    if (!offender)
      return message.channel.send("You didnt mention anyone to kick!");

    if (offender.id == message.author.id)
      return message.channel.send("Hey you cant kick yourself!");
    ...
    const success = new Discord.MessageEmbed()
      ...
      .setDescription(
        `${offender.user.username}#${offender.user.discriminator} (ID: ${offender.id}) was kicked for: ${reason}`
      )
      .setTimestamp()
      .setFooter(
