...
try {
  ...
  console.log(
    "You already have a config ready! If you want to remake it, just delete config.json and rerun the command"
  );
} catch (e) {
  const questions = [
    {
    ...
    },
    {
      ...
      choices: [
        ...
        { title: "Pastel Purple", value: "#C3B1E1" },
      ],
    },
  ];
  prompts(questions).then((values) => {
    fs.writeFileSync(
      ...
      JSON.stringify(JSON.parse(JSON.stringify(values)), null, 2)
    );
  });
}
